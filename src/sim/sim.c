...
void 
momo(struct sim *s,void* args)
{	struct topology *topo = (struct topology *)args;
	//uint32_t uuid_maxx = topo->n_hosts;
	
    	struct scheduler *sch = s->evh.sch;
    	
    	printf("momo\n");
	/*
	for(uint32_t i = 1; i<=uuid_maxx; ++i)
	{
		struct node *n = NULL;
    		HASH_FIND(hh, topo->nodes, &i, sizeof(uint32_t), n);
    		n->sch = sch;
	}*/
	struct node *nodes;
	nodes = topology_nodes(topo);
	uint32_t uuid_maxx = topology_node_num(topo);
	printf("momo0\n");
	printf("%d\n",uuid_maxx);
	for(uint32_t i = 1; i<=uuid_maxx; ++i)
	{	
		...
		uint64_t uuid = (uint64_t)i;
		
		HASH_FIND(hh, nodes, &uuid, sizeof(uint64_t), tnode);
		//printf("%d\n",tnode->type);
		if(tnode->type != HOST)
			continue;
		struct host* h = (struct host*)tnode;
		
		UpSch((struct host*)tnode,sch);
